...
########dependencies=(tesseract-ocr maim notify-send xclip)
...
# /tmp/ocr.txt.txt
tesseract -l Arabic+eng "$IMAGE_FILE" "${TEXT_FILE//\.txt/}" 2> /dev/null
...
#Alterntive version using dunstify
dunstify "ocr" "$(cat $TEXT_FILE)"
# Clean up
